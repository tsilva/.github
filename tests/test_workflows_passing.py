...
from tsilva_maintain.rules.workflows_passing import WorkflowsPassingRule
...
def test_pass_when_all_success(tmp_path):
    ...
    with patch("tsilva_maintain.rules.workflows_passing.gh_authenticated", return_value=True), \
         patch("tsilva_maintain.rules.workflows_passing.get_workflow_conclusions", return_value={"CI": "success", "Release": "success"}):
...
def test_fail_when_any_failure(tmp_path):
    ...
    rule = WorkflowsPassingRule()
    with patch("tsilva_maintain.rules.workflows_passing.gh_authenticated", return_value=True), \
         patch("tsilva_maintain.rules.workflows_passing.get_workflow_conclusions", return_value={"CI": "failure"}):
        result = rule.check(repo)
...
def test_partial_failure(tmp_path):
    ...
    rule = WorkflowsPassingRule()
    with patch("tsilva_maintain.rules.workflows_passing.gh_authenticated", return_value=True), \
         patch("tsilva_maintain.rules.workflows_passing.get_workflow_conclusions", return_value={"CI": "success", "PII Scan": "failure"}):
        result = rule.check(repo)
    assert result.status == Status.FAIL
...
def test_skip_when_gh_not_authenticated(tmp_path):
    repo = _make_repo(tmp_path)
    rule = WorkflowsPassingRule()
    with patch("tsilva_maintain.rules.workflows_passing.gh_authenticated", return_value=False):
        result = rule.check(repo)
...
def test_skip_when_no_completed_runs(tmp_path):
    ...
    rule = WorkflowsPassingRule()
    with patch("tsilva_maintain.rules.workflows_passing.gh_authenticated", return_value=True), \
         patch("tsilva_maintain.rules.workflows_passing.get_workflow_conclusions", return_value={}):
        result = rule.check(repo)
    assert result.status == Status.SKIP
